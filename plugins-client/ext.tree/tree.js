...
module.exports = ext.register("ext/tree/tree", {
    ...
    onReady : function() {
        var _self = this;
		this.model.data.childNodes[0].setAttribute("attr","workspace");
        ...
        for (var i = 0; i < this.expandedNodes.length; i++) {
            ...
            (davProject.realWebdav || davProject).readdir(this.expandedNodes[i], function(data, state, extra) {
                ...
                if (extra.status === 404) {
					...
					// set the project attribution for the first directory
					// Rannk
					for(var ii = 0; ii < dataXml.childElementCount; ii++){
							//alert(dataXml.childElemrntCount.childNodes[ii].getAttribute("type"));
							if(dataXml.childNodes[ii].getAttribute("type") == "folder"){
								var type_str_arr = dataXml.childNodes[ii].getAttribute("path").split("/");
								if(type_str_arr.length == 3)
									dataXml.childNodes[ii].setAttribute("attr","project");
							}
					}
    ...
    refresh : function(){
        ...
        //    ide.projectName + "' path='" + ide.davPrefix + "' root='1' attr='workspace'/></data>");
        this.expandedList = {};
	...
	//var selectArray = apf.activeElement.selection;	
	apf.activeElement.remove();
	util.ddProjectSelectorRefreshFlag = 1;
