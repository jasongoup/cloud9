...
module.exports = {
    ...
    saveToFile : function() {
		...
		// encode newline symbol for package's textarea
		// edit by rannk
		var op_array = new Array("preinst","postinst","prerm","postrm");

		for(var v=0;v<op_array.length;v++){
			var key = op_array[v];
			for(var i=0;i<tmp_model.getElementsByTagName("package").length;i++){
				var tmp = tmp_model.getElementsByTagName("package")[i].getAttribute(key);
				if(tmp == null) continue;
				var vv = tmp.replace(/\n/g,"|**|");
				tmp_model.getElementsByTagName("package")[i].setAttribute(key,vv);
			}
		}
		
        var data = tmp_model && apf.xmldb.cleanXml(tmp_model.xml) || "";
		...
		for(var v=0;v<op_array.length;v++){
			...
			for(var i=0;i<tmp_model.getElementsByTagName("package").length;i++){
				var tmp = tmp_model.getElementsByTagName("package")[i].getAttribute(key);
				if(tmp == null) continue;
				var vv = tmp.replace(/\|\*\*\|/g,"\n");
				tmp_model.getElementsByTagName("package")[i].setAttribute(key,vv);
			}
		}
		
		//this.model.data.getElementsByTagName("package")[0].setAttribute("preinst",tmp_str);
