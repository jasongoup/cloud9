...
var errArray = new Object();
    errArray['addProject'] ={"name":"project with same path already exists","color":"#ff0000"};
    errArray['addPackage'] = "addPackage error";
    errArray['addFileToPkg_file'] = "Invalid source file ";
    errArray['addFileToPkg_filelen'] = "Duplicate file ";
*/
...
module.exports = ext.register("ext/console/console", {
    ...
    hook: function() {
        ...
        var cmd = {
			...
			/* remove other version control cmd
			 * rannk
            "Hg" : [
                ["Sum", "hg sum"],
                ["Push", "hg push"],
                ...
                menus.addItemByPath("Tools/" + c + "/" + def[0],
                    new apf.item({
                        onclick : function(){
							...
							var xmlNode = trFiles.selected;
							var git_dir = "";
							var cmd_str = "";
							
							if(xmlNode.getAttribute("attr") == "project"){
								git_dir = "--git-dir='" + xmlNode.getAttribute("name") + "/.git' " + "--work-tree='" + xmlNode.getAttribute("name") + "'";
							}
							
							if(git_dir){
								var str_arr = def[1].split(" ");
								for(var i=0; i<str_arr.length; i++)
									if(i==0)
										cmd_str = str_arr[i] + " " + git_dir + " ";
									else
										cmd_str += str_arr[i] + " ";
							}else{
								cmd_str = def[1];	
                            ...
                            txtConsoleInput.setValue(cmd_str);
	...
	//tabConsole.activepage = "pgPkgmgrlog";
	//tabConsole.setAttribute('activepage','pgPkgmgrlog');
...
//        }), winDbgConsole);
	...
	var c9shell1 = tabConsole.add("logger");
        c9shell1.setAttribute("closebtn", true);
        var c9shellText1 = c9shell.appendChild(new apf.text({
            margin     : "3 0 0 0",
            anchors    : "0 17 0 0",
            flex       : "1",
            scrolldown : "true",
        ...
        c9shell1.appendChild(new apf.scrollbar({
            "for"     : c9shellText,
            right     : "0",
            top       : "0",
            bottom    : "0",
    ...
    pkglog:function(color,type,text){


	   var span_head = '<span style='+ "\"" + "color:";
	   var span_mid = "\""+ ">";
	   var span_tail = '</span><br>';
	   var content = span_head + color +span_mid + "infotype:"+type+",  info:" + text +span_tail;

	  // Console.pkglog('<span style='+ "\"" + "color:"+errArray["addProject"]["color"] +"\""+ ">"+ + errArray["addProject"]["name"]+":"+path +'</span><br>')	

	   txtPkgmgrput.addValue(content);
